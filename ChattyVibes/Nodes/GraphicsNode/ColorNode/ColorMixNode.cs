...
namespace ChattyVibes.Nodes.GraphicsNode.ColorNode
{
    ...
    internal class ColorMixNode : STNode
    {
        ...
        {
            base.OnCreate();
            //TitleColor = Color.FromArgb(200, FrmBindingGraphs.C_COLOR_COLOR);
            Title = "Mix RGB";
            AutoSize = false;
            Width = 140;
            Height = 127;
            ...
            m_in_color1 = InputOptions.Add("Color1", typeof(Color), true);
            m_in_color2 = InputOptions.Add("Color2", typeof(Color), true);
            ...
            m_out_color.TransferData(Color.LightGray);
        ...
        void ProcessColor()
        {
            ...
            switch (_mixType)
            {
                case ColorMixType.Add:
                    {
                        tmpA = _color1.A + (int)Math.Round((_fac * _color2.A) / (float)C_RANGE);
                        tmpR = _color1.R + (int)Math.Round((_fac * _color2.R) / (float)C_RANGE);
                        tmpG = _color1.G + (int)Math.Round((_fac * _color2.G) / (float)C_RANGE);
                        tmpB = _color1.B + (int)Math.Round((_fac * _color2.B) / (float)C_RANGE);
                ...
                case ColorMixType.Multiply:
                    {
                        tmpA = (int)Math.Round((mfac * _color1.A * C_RANGE + _fac * _color2.A * _color1.A) / (float)(C_RANGE * C_RANGE));
                        tmpR = (int)Math.Round((mfac * _color1.R * C_RANGE + _fac * _color2.R * _color1.R) / (float)(C_RANGE * C_RANGE));
                        tmpG = (int)Math.Round((mfac * _color1.G * C_RANGE + _fac * _color2.G * _color1.G) / (float)(C_RANGE * C_RANGE));
                        tmpB = (int)Math.Round((mfac * _color1.B * C_RANGE + _fac * _color2.B * _color1.B) / (float)(C_RANGE * C_RANGE));
                        outColor = Color.FromArgb(tmpA, tmpR, tmpG, tmpB);
                        break;
                    }
                case ColorMixType.Screen:
                    {
                        tmpA = (int)Math.Max(C_RANGE - (((C_RANGE - _color1.A) * (C_RANGE - _color2.A)) / C_RANGE), 0.0f);
                        tmpR = (int)Math.Max(C_RANGE - (((C_RANGE - _color1.R) * (C_RANGE - _color2.R)) / C_RANGE), 0.0f);
                        tmpG = (int)Math.Max(C_RANGE - (((C_RANGE - _color1.G) * (C_RANGE - _color2.G)) / C_RANGE), 0.0f);
                        tmpB = (int)Math.Max(C_RANGE - (((C_RANGE - _color1.B) * (C_RANGE - _color2.B)) / C_RANGE), 0.0f);
                        outColor = Color.FromArgb(
                ...
                case ColorMixType.Difference:
                    {
                        tmpA = Math.Abs(_color1.A - _color2.A);
                        tmpR = Math.Abs(_color1.R - _color2.R);
                        tmpG = Math.Abs(_color1.G - _color2.G);
                        tmpB = Math.Abs(_color1.B - _color2.B);
                        outColor = Color.FromArgb(
                            (mfac * _color1.A + tmpA * _fac) / C_RANGE,
                            (mfac * _color1.R + tmpR * _fac) / C_RANGE,
                            (mfac * _color1.G + tmpG * _fac) / C_RANGE,
                ...
                case ColorMixType.Darken:
                    {
                        if (_fac >= C_RANGE)
                        {
                            outColor = Color.FromArgb(_color2.ToArgb());
                            break;
                        }
                        ...
                        {
                            outColor = Color.FromArgb(_color1.ToArgb());
                            break;
                        }

                        tmpA = (int)Math.Round((mfac * _color1.A + _fac * _color2.A) / (float)C_RANGE);
                        tmpR = (int)Math.Round((mfac * _color1.R + _fac * _color2.R) / (float)C_RANGE);
                        tmpG = (int)Math.Round((mfac * _color1.G + _fac * _color2.G) / (float)C_RANGE);
                        tmpB = (int)Math.Round((mfac * _color1.B + _fac * _color2.B) / (float)C_RANGE);
                        outColor = Color.FromArgb(tmpA, tmpR, tmpG, tmpB);
                        break;
                    }
                ...
                case ColorMixType.Overlay:
                    {
                        int ProcChannel(int a, int b)
                        {
                            ...
                            return (int)Math.Min((mfac * a + tmp * _fac) / C_RANGE, (float)C_RANGE);
                        }
                ...
                case ColorMixType.ColorDodge:
                    {
                        ...
                        tmpA = (_color2.A == C_RANGE) ? C_RANGE
                            : (int)Math.Min((_color1.A * dodgefac) / (C_RANGE - _color2.A), (float)C_RANGE);
                        tmpR = (_color2.R == C_RANGE) ? C_RANGE
                            : (int)Math.Min((_color1.R * dodgefac) / (C_RANGE - _color2.R), (float)C_RANGE);
                        tmpG = (_color2.G == C_RANGE) ? C_RANGE
                            : (int)Math.Min((_color1.G * dodgefac) / (C_RANGE - _color2.G), (float)C_RANGE);
                        tmpB = (_color2.B == C_RANGE) ? C_RANGE
                            : (int)Math.Min((_color1.B * dodgefac) / (C_RANGE - _color2.B), (float)C_RANGE);
                        outColor = Color.FromArgb(
                            (mfac * _color1.A + tmpA * _fac) / C_RANGE,
                            (mfac * _color1.R + tmpR * _fac) / C_RANGE,
                            (mfac * _color1.G + tmpG * _fac) / C_RANGE,
                ...
                case ColorMixType.ColorBurn:
                    {
                        ...
                        {
                            int tmp = (b == 0) ? 0
                                : Math.Max(C_RANGE - ((C_RANGE - a) * C_RANGE) / b, 0);
                        ...
                        tmpG = ProcChannel(_color1.G, _color2.G);
                        tmpB = ProcChannel(_color1.B, _color2.B);
                        outColor = Color.FromArgb(_color1.A, tmpR, tmpG, tmpB);
                        break;
                    }
                ...
                case ColorMixType.SoftLight:
                    {
                        int add = (int)Math.Round(C_RANGE / 4.0f);
                ...
                case ColorMixType.LinearLight:
                    {
                        ...
                        {
                            int tmp = (b > cmp)
                                ? Math.Min(a + 2 * (b - cmp), C_RANGE)
                                : Math.Max(a + 2 * b - C_RANGE, 0);
                            return (int)((tmp * _fac + a * mfac) / C_RANGE);
                        }
        ...
        private static byte GetLuminance(Color rgb) =>
        ...
        private static void RgbToHsv(float r, float g, float b, ref float r_h, ref float r_s, ref float r_v)
        {
            ...
            r_h = Math.Abs(k + (g - b) / (6.0f * chroma + 1e-20f));
            r_s = chroma / (r + 1e-20f);
            r_v = r;
        }
        ...
        private static void HsvToRgb(float h, float s, float v, ref float r_r, ref float r_g, ref float r_b)
        {
            ...
            nr = Math.Abs(h * 6.0f - 3.0f) - 1.0f;
            ng = 2.0f - Math.Abs(h * 6.0f - 2.0f);
            nb = 2.0f - Math.Abs(h * 6.0f - 4.0f);
        ...
        private void InNumDataTransfer(object sender, STNodeOptionEventArgs e)
        {
            if (e.Status == ConnectionStatus.Connected && e.TargetOption.Data != null)
                Fac = ((int)Math.Round((float)e.TargetOption.Data * C_RANGE)).Clamp(0, C_RANGE);
            else
                Fac = 127;
        }
        ...
        private void InColorDataTransfer(object sender, STNodeOptionEventArgs e)
        {
            if (e.Status == ConnectionStatus.Connected && e.TargetOption.Data != null)
            {
                if (sender == m_in_color1)
                    Color1 = (Color)e.TargetOption.Data;
                else
                    Color2 = (Color)e.TargetOption.Data;
            }
            else
            {
                if (sender == m_in_color1)
