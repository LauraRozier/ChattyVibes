...
{
    internal abstract class BranchNode : STNode
    {
        ...
        protected STNodeOption m_op_in;
        ...
        {
            base.OnCreate();
            TitleColor = Color.FromArgb(200, FrmBindingGraphs.C_COLOR_BRANCH);
        ...
        protected override void OnDrawOptionText(DrawingTools dt, STNodeOption op)
        {
            ...
            if (op == m_op_true_out)
            {
                dt.Graphics.DrawString("══╦", Font, Brushes.ForestGreen, rect, _sf);
            ...
            else if (op == m_op_false_out)
            {
                dt.Graphics.DrawString(
                    "  ║", Font, _condition ? Brushes.White : Brushes.ForestGreen,
                    new Rectangle(rect.X, rect.Y - 10, rect.Width, rect.Height), _sf
                );
